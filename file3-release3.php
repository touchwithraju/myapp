...
class FileUploader {
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            //'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/uploads/cloud_files/',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')) . '/uploads/cloud_files/',
            'upload_url' => $this->get_full_url() . '/uploads/cloud_files/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
            ...
            'inline_file_types' => '/\.(gif|jpe?g|png|mp3|mp4|mov|pdf)$/i',
            ...
            'image_file_types' => '/\.(gif|jpe?g|png|pdf|mp3|mp4|mov)$/i',
    ...
    protected function set_additional_file_properties($file) {
        ...
        $file->deleteUrl = $this->options['script_url'] . 'uploader/deleteFile/' . rawurlencode($file->name);
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
        ...
        $rename = $this->randomString() . '_' . time() . '.' . $file_parts['extension'];
        $file = new stdClass();
        $file->name = $this->get_file_name($uploaded_file, $rename, $size, $type, $error, $index, $content_range);
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    $arr_parts = explode('/', $file_path);
                    $fname = $arr_parts[count($arr_parts) - 1];
                    //ini_set('max_upload_filesize', 20971520);
                    move_uploaded_file($uploaded_file, $file_path);
                    $file_parts = pathinfo($file_path);
                    $path_parts = pathinfo($name);
                    $file_nm = $path_parts['filename'];
                    // check if file name has "_1" in it
                    $group_file_parts = explode('_', $file_nm);
                    $search_name = $file_nm;
                     if (count($group_file_parts) > 1) {
                        $lst_prt = array_pop($group_file_parts);
                        if (is_numeric($lst_prt)) {
                            $search_name = implode('_', $group_file_parts);
                        }
                    }
                    $dyn_str = $search_name . '_' . implode('_', $_POST);
                    $group_col = md5($dyn_str);
                    $iparray['post'] = $_POST;
                    // $this->deleteDuplicate($group_col,$name);
                    $iparray['file_details'] = array('attachment_orig_name' => $name,
                        'attachment_rename' => $rename,
                        'rename' => $file_parts['basename'],
                        'ipaddress' => $_SERVER['REMOTE_ADDR']);
                    $iparray['post'] = $_POST;
					//echo '';
                    $file->att_id = $this->inserAttachment($iparray);
                }
            } else {
    ...
    function inserAttachment($iparray) {
        $CI = & get_instance();
        $file_details = $iparray['file_details'];
        $post = $iparray['post'];
        $arr_user_details = @unserialize($CI->db_session->userdata('user_login_details'));
        $user_id = $arr_user_details['id'];
        // frame tag_array
        $arr_tg = array();
        ...
        $filename = $file_details['attachment_orig_name'];
        $path_parts = pathinfo($filename);
        $ext = strtolower($path_parts['extension']);
        $file_nm = $path_parts['filename'];
        // check if file name has "_1" in it
        $file_parts = explode('_', $file_nm);
        $search_name = $file_nm;
        $parent_id = 0;
        if (count($file_parts) > 1) {
            $lst_prt = array_pop($file_parts);
            if (is_numeric($lst_prt)) {
                $search_name = implode('_', $file_parts);
            }
        }
        $arr_val_ref = $this->getValReference($CI);
        $arr_tag_ref = $this->getTagReferences($CI);
        $arr_all_tag_ref = $this->getTagAllReferences($CI);
        $att_id=0;
        $astatus=true;
        $dyn_str = $search_name . '_' . implode('_', $post);
        $group_col = md5($dyn_str);
        $asql=$CI->db->query("select attachment_rename from attachments where attachment_orig_name='".mysql_real_escape_string($filename)."' and group_col='".$group_col."'");
        if($asql->num_rows>0)
        {
          $astatus=false;  
        }
        ...
        foreach ($arr_tg as $tag => $val) {
            if (isset($arr_tag_ref[$tag])) {
                $att_col .= ',' . $tag;
                $att_val .= ',"' . $val . '"';
                $att_val_dublicate .= ',"'.$arr_val_ref[$val].'"';
            }
        }
            if($astatus){
       $sql = 'INSERT INTO `attachments` (`cstatus`,`extension`,`file_rename`,`base_name`,`group_col`,`attachment_orig_name`, `attachment_rename`, `project_id`, `created_by`, `created_date`, `ipaddress`' . $att_col . ') VALUES ("2","' . $ext . '","' . $file_details['rename'] . '","' . $search_name . '","' . $group_col . '","' . $filename . '", "' . $file_details['attachment_rename'] . '","' . $project_id . '","' . $user_id . '","' . date('Y-m-d H:i:s') . '","' . $file_details['ipaddress'] . '"' . $att_val_dublicate . ');';
       $query_object = $CI->db->query($sql);
        $att_id = $CI->db->insert_id();
        foreach($arr_tg as $tag=>$val){
			$tag_qry .= '('.$att_id.','.$arr_all_tag_ref[$tag].','.$val.','.$user_id.',"'.date('Y-m-d H:i:s').'","'.$_SERVER['REMOTE_ADDR'].'"),';
		}
                //echo $sql;
               // die;
        ...
        // insert attachments
        $sql = 'INSERT INTO `cloud_attachments` (`attachment_id`,`extension`,`file_rename`,`base_name`,`group_col`,`attachment_orig_name`, `attachment_rename`, `project_id`, `created_by`, `created_date`, `ipaddress`' . $att_col . ') VALUES ("' . $att_id . '","' . $ext . '","' . $file_details['rename'] . '","' . $search_name . '","' . $group_col . '","' . $filename . '", "' . $file_details['attachment_rename'] . '","' . $project_id . '","' . $user_id . '","' . date('Y-m-d H:i:s') . '","' . $file_details['ipaddress'] . '"' . $att_val . ');';
        $query_object = $CI->db->query($sql);
    }
   function getValReference($CI) {
    ...
    function getTagAllReferences($CI) {
    ...
    function randomString($length = 5) {
        $random = '';
        $CI = & get_instance();
        $user_details = @unserialize($CI->db_session->userdata('user_login_details'));
